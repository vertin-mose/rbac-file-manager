...
| **L4** | EDITOR | 文档编辑员 | 中 | 文档内容创作者，创建、编辑、共享文档，参与评论 | 总体数据 + 文件管理（新建/编辑按钮） | 普通员工 |
...
> **职责分离原则**：EDITOR 和 REVIEWER 处于同一层级（L4），但权限互不重叠。EDITOR 不能自审自批，REVIEWER 不能编辑内容。MANAGER（L3）继承两者权限，可在必要时覆盖操作。
...
> 项目状态：后端 API 全部完成，前端仅登录页可用，数据库就绪，集成测试脚本可用。
...
所有 22 个 API 端点已在 `backend/main.py` 中实现，含完整 RBAC 权限检查：

| 模块 | API 端点 | 文件位置 |
|------|----------|----------|
| **认证** | 登录 / 注册 / 登出 | `backend/main.py` → `services.py` (auth_service) |
| **角色管理** | 角色 CRUD、层级查询、权限分配、用户角色分配 | `backend/main.py` → `services.py` (role_service) |
| **文档管理** | 文档列表/详情、上传、目录创建、重命名、删除、共享、审阅、审批、评论 | `backend/main.py` → `services.py` (file_service) |
...
| 页面/组件 | 文件位置 | 状态详情 |
|-----------|---------|----------|
| 登录页 | `frontend/src/views/login/LoginView.vue` | 完整实现，支持用户名/密码登录，登录后跳转总体数据 |
| 路由守卫 | `frontend/src/router/index.ts` | 已配置路由守卫，未登录自动跳转登录页 |
| API 请求封装 | `frontend/src/api/request.ts` | Axios 封装，含 JWT Token 自动附加、401 自动跳转登录 |
| 布局组件 | `frontend/src/components/AppLayout.vue` | 侧边栏菜单（按角色动态显示）+ 顶部导航栏（含角色身份标签） |
| 用户状态管理 | `frontend/src/store/user.ts` | 存储 Token/用户名/角色/权限/角色信息，支持 `hasPermission()` 细粒度权限检查 |
| 总体数据页 | `frontend/src/views/dashboard/DashboardView.vue` | 四个统计卡片（文件数、用户数、角色数、存储用量） |
...
| 页面/组件 | 文件位置 | 需要实现的内容 |
|-----------|---------|---------------|
| **文件管理器** | `frontend/src/views/file/FileManager.vue` | 目录树、面包屑导航、文件列表、拖拽上传、右键菜单 |
| **目录树组件** | `frontend/src/components/FileTree.vue` | 递归展示、懒加载、节点操作（新建/重命名/删除） |
| **角色管理** | `frontend/src/views/role/RoleManagement.vue` | 角色 CRUD 表格、权限矩阵勾选、用户-角色关联 |
| **审计日志查看器** | `frontend/src/views/audit/AuditLogView.vue` | 分页表格、操作类型筛选、日期范围筛选、CSV 导出 |
| **注册页面** | 需新建页面 | 后端注册 API 已可用，前端缺注册表单 |
| **权限按钮级控制** | 前端各视图 | 根据用户 `hasPermission()` 动态显示/隐藏界面操作按钮（文件管理已实现） |
...
| `file_records` | 文件记录表（文件名、类型、大小、父目录、上传者、MinIO 路径） |
| `file_permissions` | 文件级权限控制 |
...
| POST | `/api/files/{id}/approve` | 审批文档 | doc:approve |

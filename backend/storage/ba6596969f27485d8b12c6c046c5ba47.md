...
| **文档管理** | 文档列表/详情/下载、上传、目录创建、重命名、删除、共享、审阅、审批、评论 | `backend/main.py` → `services.py` (file_service) |
| **审计日志** | 日志查询（分页+筛选）、CSV 导出 | `backend/main.py` → `services.py` (audit_service) |
| **用户信息** | 用户信息查询（按 ID） | `backend/main.py` → `services.py` (role_service) |
...
| **文件管理器** | `frontend/src/views/file/FileManager.vue` | 目录树 + 面包屑导航 + 文件列表 + 文件上传 + 创建目录 + 重命名 + 删除 + 共享/审阅/审批/评论弹窗 + 权限按钮控制 |
| **目录树组件** | `frontend/src/components/FileTree.vue` | 递归展开全部目录、点击节点跳转、内联新建/重命名/删除操作 |
| **角色管理** | `frontend/src/views/role/RoleManagement.vue` | 角色 CRUD 表格 + 权限矩阵勾选（按分类分组）+ 角色层级时间线展示 + 用户-角色分配 |
...
| 用户角色分配需手动输入用户 ID | 后端没有提供完整用户列表接口供前端下拉选择，但已实现用户信息查询（`GET /api/users/{id}`），输入 ID 后自动显示对应用户信息并需二次确认 |
| 后端部分接口存在 async/await 缺失 | `api_create_directory`、`api_rename_file`、`api_delete_file`、`api_share_file`、`api_review_file`、`api_approve_file`、`api_comment_file` 共 7 个路由中 `get_current_user()` 和 `require_perm()` 调用缺少 `await`，可能导致操作时 500 错误 |

...
│   ├── models.py                    # 数据模型：10 个 SQLAlchemy ORM 模型（User、Role、Permission 等）
...
│   │                                # MinIO 单例懒加载：仅首次调用时检测连接，后续缓存避免超时
...
│   │                                # role_hierarchy、file_records、file_permissions、file_activities、audit_logs
...
| **L2** | ADMIN | 系统管理员 | 高 | 组织管理者，负责用户管理、角色分配、审计导出与文件授权管理 | 总体数据 + 文件管理 + 角色管理 + 审计日志 | IT 运维人员 |
...
```
                        ...
                        └────────┬────────┘
                                 │ inherits
                        ┌────────▼────────┐
                        │     ADMIN       │  ← 继承 MANAGER 的全部权限 + user/role 管理 + audit 导出 + file:permission:manage
                        └────────┬────────┘
                                 │ inherits
                        ┌────────▼────────┐
...
- 数据库 ORM 模型：`backend/models.py` — 10 个 SQLAlchemy 模型 + Pydantic 数据模型
...
| 问题 | 说明 |
|------|------|
| 上传性能（非 Docker 环境） | 本地开发时 MinIO 通常未运行，上传会走本地文件系统 fallback。已优化为全局性单例检测（`get_minio_client()` 仅首次调用时尝试连接，之后直接缓存结果），大幅减少等待时间 |
